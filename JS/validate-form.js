...
function validateForm() {
  ...
  if (userPhone.value.trim() == '') {
    ...
    userPhone.value.trim().length < 5 ||
    userPhone.value.trim().lenghth > 11
  ) {
    setError(userPhone, 'enter a valid number!')
  } else {
  ...
  if (userMsg.value.trim() == '') {
    ...
    userMsg.value.trim().lenghth > 300
  ) {
    setError(userMsg, 'enter a valid text message!')
  } else {
    setSuccess(userMsg)
  }
...
function redirect_chat_zone() {
  let chat_url =
    'http://localhost/xampp/PHP-NEW-PROJECT/FullStack-Blog-App/Assets/Admin/community.php'
